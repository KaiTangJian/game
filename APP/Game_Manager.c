...
static bool is_move_valid(const GamePlayer_t *player, int8_t dx, int8_t dy) {
    ...
    // 4. 特定玩家类型与危险地形的交互检查
    if (player->type == PLAYER_TYPE_ICE) { // 如果是冰人
        // 冰人不能进入岩浆或火焰瓦格
        // TODO: 定义 TILE_TYPE_LAVA，目前你的 TileType_t 中只有 FIRE/ICE/NORMAL/EXIT...
        // 假设 TILE_TYPE_LAVA 是存在的
        if (target_tile == TILE_TYPE_FIRE) {
            return false;
        }
    } else { // 如果是火人 (PLAYER_TYPE_FIRE)
        // 火人不能进入水域或冰川瓦格
        // TODO: 定义 TILE_TYPE_WATER，目前你的 TileType_t 中只有 FIRE/ICE/NORMAL/EXIT...
    ...
    // 5. 其他更复杂的碰撞检查 (根据游戏设计添加)

    return true; // 所有检查通过，移动有效
}
...
//        // 检查并处理宝石收集
//        // 注意: map_data 通常是 const（只读）。如果需要移除收集到的宝石，
//        // 需要一个单独的机制来管理地图上的动态元素，而不是直接修改 map_data。
