...
typedef enum {
    ...
    TILE_TYPE_NORMAL,   // 正常地面
		TILE_TYPE_AIR,		//空气
    TILE_TYPE_EXIT,     // 出口
    TILE_TYPE_COLLECTIBLE_FIRE_GEM, // 火宝石
    TILE_TYPE_COLLECTIBLE_ICE_GEM,  // 冰宝石
...
typedef struct GamePlayer 
{
    ...
    int health;            // 生命值 (例如，100点)
    uint32_t score;        // 当前分数
    uint8_t lives;         // 剩余生命数
    bool is_alive;         // 是否存活
    bool is_invincible;    // 是否处于无敌状态
    float vertical_velocity; // 当前垂直速度 (负数向上，正数向下)
    bool on_ground;          // 是否站在地面上
...
typedef struct {
    ...
    Point_t player1_start; // 冰人开始
    Point_t player2_start; // 火人开始
